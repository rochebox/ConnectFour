...
public class Board_5_15_Will_Charles extends JPanel implements MouseListener, MouseMotionListener, ActionListener {
	...
	//Constructor
	public Board_5_15_Will_Charles (int w, int h) {
		pWidth = w;
		pHeight = h;
	...
	private void saveWinningMoves(int[] rows, int[] cols) {
		winMoveList = new ArrayList<Point>();
		
		for(int i = 0; i < rows.length; i++) {
			winMoveList.add(new Point( cols[i], rows[i]));
		}
	}
	...
	public boolean winCheck() {
		//With code added to get win moves
			...
			for (int j = 0; j < COLS; j++) {
				...
				if (j <= COLS - 4) {
					if (Math.abs(matrix[i][j] + matrix[i][j+1] + matrix[i][j+2] + matrix [i][j+3]) == 4) {
						win = true;
						int[] winningRows = {i, i, i, i};
						int[] winningCols = {j, j+1, j+2, j+3};
						saveWinningMoves(winningRows, winningCols);
						break;
					}
				}
				...
				if (i <= ROWS - 4) {
					if (Math.abs(matrix[i][j] + matrix[i+1][j] + matrix[i+2][j] + matrix [i+3][j]) == 4) {
						win = true;
						int[] winningRows = {i, i+1, i+2, i+3};
						int[] winningCols = {j, j, j, j};
						saveWinningMoves(winningRows, winningCols);
						break;
					}
				}
